...
class block_us_faq{
    ...
    public function block_us_faq_render($block, $content = '', $is_preview = false){
        ...
        wp_enqueue_style("block_us_faq", get_template_directory_uri()."/css/block_faq.css");
        wp_enqueue_script("block_us_faq", get_template_directory_uri()."/js/block_faq.js");
        $c = "";
        ...
        if($faqs && count($faqs)){
            ...
            foreach($faqs AS $faq){
                $theHTML .= '<div class="faq-item'.(($faqFirstOpen && !$i) ? ' active' : '').'">';
                    ...
                    $theHTML .= '<div class="faq-answer"'.(($faqFirstOpen && !$i) ? ' style="display: block;"' : '').'>'.$faq["answer"].'</div>';
                $theHTML .= '</div>';
                $i++;
            }
            $theHTML .= '</div>';
